...
async function addFavicon(userId, faviconObject) {
  ...
  let ownerRef = await userFaviconRef
    .where("owner", "==", userId)
    .where("userId", "==", faviconObject["userId"])
    .get();
...
async function getFaviconList(userId) {
  let faviconList = await db
    .collection(DB_COLLECTION["FAVICON"])
    .where("owner", "==", userId)
    .get();
  return new Promise((resolve) => {
    let faviconObj = {};
...
async function transferHSC(senderAddress, receiverAddress, amount) {
  let decimals = await hsContract.methods.decimals().call();
  decimals = parseInt(decimals);
  let transactionObj = await hsContract.methods
    .transfer(receiverAddress, new BigNumber(amount * 10 ** decimals))
  ...
  putItemToDB(DB_COLLECTION["TRANSACTION_HASH"], currTimeMilli, {
...
async function remittanceCoin(senderAddress, receiverAddress, amount) {
  ...
  if (
    (await isAddressInDB(senderAddress)) &&
    (await isAddressInDB(receiverAddress))
  ) {
    ...
    if (senderBalance >= amount) {
      ...
      console.log(
        `### remittance return code SUCCESS(${RETURN_CODE["SUCCESS"]})`
      );
    } else {
      console.log("### remittance balance check false");
      resultCode = RETURN_CODE["NOT_ENOUGH_MONEY"];
      console.log(
        `### remittance return code NOT_ENOUGH_MONEY(${RETURN_CODE["NOT_ENOUGH_MONEY"]})`
      );
    }
  } else {
    console.log("### remittance balance check false");
    ...
    console.log(
      `### remittance return code NONE_ADDRESS(${RETURN_CODE["NONE_ADDRESS"]})`
    );
  }
...
async function paymentCoin(senderAddress, receiverAddress, amount) {
  ...
  if (
    ...
    if (senderBalance >= amount) {
      ...
      resultCode = RETURN_CODE["NOT_ENOUGH_MONEY"];
      console.log(
        `### payment return code NOT_ENOUGH_MONEY(${RETURN_CODE["NOT_ENOUGH_MONEY"]})`
      );
    }
  } else {
    console.log("### payment balance check false");
    resultCode = RETURN_CODE["NONE_ADDRESS"];
    console.log(
      `### payment return code NONE_ADDRESS(${RETURN_CODE["NONE_ADDRESS"]})`
    );
  }
...
async function isAddressInDB(inquiryAddress) {
  ...
  let snapShot = await userInfoRef
    .where("accountAddress", "==", inquiryAddress)
    .get();
...
async function isPasswordRight(userId, userPassword) {
  ...
  console.log(
    `### userId = ${userId}, userPassword = ${userPassword} isSuccess ${isSuccess}`
  );
  return isSuccess;
}
    ...
    ps.onSnapshot(
      (docSnapshot) => {
        try {
          ...
          userid = docSnapshot["_fieldsProto"]["userid"]["stringValue"];
          userAccount =
            docSnapshot["_fieldsProto"]["accountAddress"]["stringValue"];
 ...
 * @param {string} day
 */
async function userSignUp(userid, userpw, username, useremail, userphone) {
  var year = 1997;
  var month = 10;
  var day = 10;
...
async function getTransactionLog(inquiryAddress) {
  ...
  return new Promise(async (resolve) => {
    await transactionRef
      .where("receiverAddress", "in", [inquiryAddress])
      .get()
      .then((res) => {
        res.forEach((result) => {
    ...
    await transactionRef
      .where("senderAddress", "in", [inquiryAddress])
      .get()
      .then((res) => {
        res.forEach((result) => {
          logObj[result.id] = result.data();
